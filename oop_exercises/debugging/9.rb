...
=begin
The problem is that the Mailing module, which contained the #send method, 
was not mixed in to teh PostalService class, only the EmailService class.
Both of these classes are dealing with mail type behaviors, so Mailing should be
mixed in to both. Telephone class does not deal with mailing, so it is not necessary to mix it in there.

Within the Mailing module, we define the #send method which is defined in the Object
class. To avoid overriding this mehtod, we should define a method with a more
specific name, such as #send_mail
=end